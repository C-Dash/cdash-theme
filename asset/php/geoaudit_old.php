...
if ($folder_set) {
    ...
    // Get the row count
    $foldercount = $row['rowcount']; // Access the count using the alias 'row_count'
...
// Count Places as rows in CDASH_Places_View

$count_places = $conn->query("SELECT count(*) as rowcount from cdash_places_view");

if ($count_places) {
    // Fetch the result as an associative array
    $count_places_result = $count_places->fetch_assoc();
    // Get the row count
    $place_count = $count_places_result['rowcount']; // Access the count using the alias 'row_count'
    mysqli_commit($conn);
...
$count_docs = $conn->query("SELECT count(*) as rowcount from cdash_docs_view");
if ($count_docs) {
    // Fetch the result as an associative array
    $row = $count_docs->fetch_assoc();
    // Get the row count
    $doccount = $row['rowcount']; // Access the count using the alias 'row_count'
...
if ($count_items) {
    ...
    // Get the row count
    $itemcount = $row['rowcount']; // Access the count using the alias 'row_count'
...
$count_places_wno_docs_sql = "select resource.title as Title, resource.id as resource_id, cdash_docs_view.doc_id as doc_id from resource
left outer join cdash_docs_view on resource.id = cdash_docs_view.place_id 
WHERE resource_template_id = (select id from resource_template where label = 'CDASH Place') and cdash_docs_view.place_id IS NULL;";
...
if ($result) {
    ...
    echo "<table>";
       while ($row = $result->fetch_assoc()) {
         ...
         //echo arrayToHtmlTable($result->fetch_all());
        }
        echo"</table></div>";
    }
} else {
...
left outer join cdash_docs_view on resource.id = cdash_docs_view.doc_id 
WHERE resource_template_id = (select id from resource_template where label = 'CDASH Document') and cdash_docs_view.place_id IS NULL;";
...
// select * from cdash_docs_view where place_lng is null;

// // Places without Markers
// select * from cdash_places_view where lng is null;

// // Documents without Streetsort
// select doc_id from cdash_docs_view where streetsort is null;

// // Places without Streetsort
// select item_id from cdash_places_view where streetsort is null;

// // select folders that have no documents
// $foldercout_sql select doc_id from cdash_docs_view 
// left outer join cdash_places_view on cdash_places_view.item_id = cdash_docs_view.place_id
// where cdash_places_view.item_id IS NULL

// // Documents not associated with Places
// select doc_id, doctitle from cdash_docs_view 
// left outer join cdash_places_view on cdash_places_view.item_id = cdash_docs_view.place_id
// where cdash_places_view.item_id IS NULL

// // Places that have no Documents
// select place_id, placeItem from cdash_places_view 
// left outer join cdash_docs_view on cdash_places_view.item_id = cdash_docs_view.place_id
// where cdash_docs_view.place_id IS NULL
